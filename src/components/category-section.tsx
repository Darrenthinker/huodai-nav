...
export function CategorySection({ category, sites, query, onClickTrack }: Props) {
  ...
  return (
    <section id={`cat-${category.id}`} className="mb-10 scroll-mt-20">
      <h2 className="flex items-center gap-2 text-[15px] font-semibold text-[#1d1d1f] mb-4">
        <span className={`w-7 h-7 rounded-lg flex items-center justify-center ${getCategoryBadge(category.name)}`}>
          <Icon size={14} />
        </span>
        {category.name}
        <span className="text-[12px] font-normal text-[#86868b] ml-0.5">{sites.length}</span>
      </h2>
